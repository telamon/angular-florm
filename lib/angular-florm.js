(function(window,angular,undefined){
	...
	.provider('florm',function FlormProvider(){
		this.defaults = {
			prefix: 'florm'
		};

		var defaults = this.defaults;
		/* The serializer is an adapter between the storage and florm's
		* get/set methods, You can provide a custom serializer as long as it
		* implements methods, `get(key,fallbackValue)` ,`set(key,value)`  `and clear()` 
		*/
		this.defaults.serializer=  function(storage){
			return {
				set:function(key,value){
					storage.setItem(defaults.prefix+key,JSON.stringify(value));
					return value;
				},
				get: function(key,def){
					return JSON.parse(storage.getItem(defaults.prefix+key)) || def;
				},
				clear: function(){
		...
		this.$get = ['LocalStorage',function flormFactory(LocalStorage){
			defaults.storage = defaults.storage || LocalStorage;
			var serializer = defaults.serializer(defaults.storage);
			var guid = function(){
			...
			var florm=  function(ns,sub){
				typeof serializer.get(ns) === 'object' || serializer.set(ns,{});
				var ProtoBase = {
					fields: {
						id: 'String',
						createdAt : 'Date',
					...
					all : function(terms){
						...
						if(typeof terms === 'object'){
							results = where(values(serializer.get(ns)),terms);
						}else{
							results = values(serializer.get(ns));
						}
						return results.map(
							function(i){
					...
					update: function(){
						if(typeof arguments[0] === 'string'){
							return this.find(arguments[0]).update(arguments[1]);
						}
					...
					find: function(terms){
						...
						if(typeof terms === 'string'){
							result = this.__inst(serializer.get(ns)[terms]);
						}else if(typeof terms === 'object'){
							result = this.__inst(findWhere(values(serializer.get(ns)),terms));
						}else{
	...
	.factory('LocalStorage',function($window){
		...
		try{$window.localStorage}catch(e){
			if(e.match(/Use chrome\.storage\.local/)){
